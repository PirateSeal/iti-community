  ...
  map(data: PostData): Post {
    return {
      ...
      message: this.parseMessage(
        `${data.message} ${data.attachementUrl ? data.attachementUrl : ''}`
      ),
    };
  }

  private parseMessage(message: string): PostMessage {
    const pictureRegex = /http[s]?:\/\/.+\.(jpeg|jpg|gif|png)/gim;

    const videoRegex = /http[s]?:\/\/.+\.(mp4|wmv|flv|avi|wav)/gim;

    const audioRegex = /http[s]?:\/\/.+\.(mp3|ogg|wav|m4a)/gim;

    const youtubeRegex = /(http[s]?:\/\/)?www\.(?:youtube\.com\/\S*(?:(?:\/e(?:mbed))?\/|watch\/?\?(?:\S*?&?v\=))|youtu\.be\/)([a-zA-Z0-9_-]{6,11})/gim;
